...
var DateRangePicker = new Class({
	...
	onSetDate: function(formated, dates)
	{
		...
		if($(this.options.inputs[0]).onchange)
			$(this.options.inputs[0]).onchange();
		this.cancel();
	},
...
window.addEvent('domready', function(){
	...
	{
		settings_menuFx = new Fx.Slide('user_controls', {'duration': 'short', 'mode': 'vertical', onComplete: backend_menu_complete, onStart: user_menu_hide_shadow, 'wrapper': $('user_controls_wrapper')}).hide();
		var 
...
function user_menu_hide_shadow()
{
	if(settings_menuFx.open == true)
...
var BackendVScroller = new Class({
	...
	init_slider_fx: function()
	{
		this.slider_fx = new Slider(this.slider, this.knob, {
			...
			onChange: function(step){
				...
				this.element_fx_scroll.set(0, Math.ceil(effective_scroll_size*step/1000));
				jQuery(this.element).trigger('onScroll');
			}.bind(this)
		});
	...
	{
		var ev = jQuery.Event("onBeforeWheel");
		jQuery(this.element).trigger(ev);
		if (ev.isDefaultPrevented())
			return;
		
		var mode = (this.slider_fx.options.mode == 'horizontal') ? (event.wheel < 0) : (event.wheel > 0);
		
		if ($(document.body).hasClass('chrome'))
		{
			event.wheel = event.wheel < 0 ? -1 : 1;
		}

		var step_size = Math.round(event.wheel*20*1000/this.element.getScrollSize().y);

		var position = this.slider_fx.step - step_size;
		if (this.last_position === undefined || this.last_position == position)
		{
			this.slider_fx.set(position);
			jQuery(this.element).trigger('onScroll');
		}
	...
	update: function(position)
	{
		...
		if (this.element.scrollTop + element_size.y > element_scroll_size.y || (position !== undefined && position == 'bottom'))
		{
			this.slider_fx.set(1000);
			this.element_fx_scroll.toBottom();
		} else
		{
			if (this.slider_fx.step == 1000 && (element_scroll_size.y > this.element.scrollTop + element_size.y))
			{
				this.slider_fx.set(1000);
				this.element_fx_scroll.toBottom();
			}
	...
	update_position: function(position)
	{
		...
		var new_position = position === undefined ? this.element.scrollTop : position;

		if (new_position <= this.options.position_threshold)
			this.slider_fx.set(0);
		else if (max - new_position <= this.options.position_threshold)
			this.slider_fx.set(1000);
		else
			this.slider_fx.set(Math.floor(new_position/max*1000));
	}
});
	...
	$.fn.tableRowMenu = function(options) {
		...
		return this.each(function() {
			...
			function _show_menu(event){
				...
				// In IE, layerY includes window.pageYOffset value. If the script is running in IE9, adjust the positioning. Not needed for IE10.
				var is_ie = navigator.appName == "Microsoft Internet Explorer" && parseFloat(navigator.userAgent.match(/MSIE ([0-9]+[\.0-9]+)/)[1]);
				if(is_ie == 9)
					posy = posy - window.pageYOffset;

				menu.css('left', (posx + 10) + 'px');
				menu.css('top', (posy - 15) + 'px');
...
Element.implement({
	...
	select_focus: function() {
		var el = jQuery(this).parent().find('a.chosen-single');
...
function backend_focus_first(parent) {
	var el = jQuery(parent).find('input, textarea, a.chosen-single, div.checkbox').filter(":visible").filter(":first");
	if (el.length > 0) {
...
function backend_bind_default_button(parent) {
	...
	form[0].bindKeys({'ctrl+enter': function(event) {
		var overlay = form.find('.ui-overlay');
		if (overlay.length > 0)
			return false;
...
window.addEvent('popupLoaded', function(element){
	...
	var focused = backend_focus_first(element),
		button = backend_bind_default_button(element);
...
function start_backend_tour()
{
	jQuery('body').addClass('tour');
	jQuery('body').css('padding', 30);
	jQuery('#header_elements, #content').css({'position': 'relative'});
}
 
function finish_backend_tour(name)
{
	jQuery('#header_elements, #content').css({'position': 'static'});
	jQuery('body').css('padding', 0);
	jQuery('body').removeClass('tour');
	
	return $('backend-footer-form').sendPhpr('onFinishTour', {
		extraFields: {
			'name': name
		},
		loadIndicator: {show: false}
