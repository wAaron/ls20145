	...
	class Shop_Orders extends Backend_Controller
	{
		...
		protected function onLoadItemForm()
		{
			try
			{
				...
				$this->apply_single_item_discount($item);
		...
		protected function onUpdateItemPriceAndDiscount($order_id)
		{
			try
			{
				...
				if ($item && post('auto_discount_price_eval'))
				{
					...
					$customer = $this->find_customer($order);
					$customer_group_id = $this->find_customer_group_id($order);
		...
		public function formBeforeSave($order, $session_key)
		{
			...
			$shipping_methods = $this->getAvailableShippingMethods($order);
			...
			$items = $this->evalOrderTotals($order);
		...
		private function evalOrderTotals($order, $items = null)
		{
			...
			Shop_TaxClass::set_customer_context($this->find_customer($order, true));
			...
			{
				$methods = $this->getAvailableShippingMethods($order);
