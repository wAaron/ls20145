<?php
	...
	class Blog_Post extends Db_ActiveRecord
	{
		...
		public static function list_recent_posts($number = 10, $show_future_posts = true)
		{
			$posts = Blog_Post::create();
			$posts->where('is_published is not null and is_published=1');
			if(!$show_future_posts)
			{
				$current_date = Phpr_Date::userDate(Phpr_DateTime::now())->getDate();
				$posts->where('blog_posts.published_date <= ?', $current_date);
			}
		...
		{
			Backend::$events->fireEvent('blog:onAfterPostRecordFetch', $this);
		}
	}
