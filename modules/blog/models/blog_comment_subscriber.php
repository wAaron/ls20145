	...
	class Blog_Comment_Subscriber extends Db_ActiveRecord
	{
		...
		public static function send_notifications($post, $comment)
		{
			...
			foreach ($subscribers as $subscriber)
			{
				...
				$email_scope_vars['subscriber_name'] = $subscriber->subscriber_name;
				$email_scope_vars['subscriber_email'] = $subscriber->email;
				$email_scope_vars['unsubscribe_link'] = $unsubscribe_url;

				// init email subject and text
				$template_subject = $template->subject;
				$template_text = $template->content;

				// eval compound vars, taking into account email scope vars
				$template_subject = System_CompoundEmailVar::apply_scope_variables($template_subject, 'blog:blog_variables', $email_scope_vars);
				$template_text = System_CompoundEmailVar::apply_scope_variables($template_text, 'blog:blog_variables', $email_scope_vars);
