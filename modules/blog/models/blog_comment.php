	...
	class Blog_Comment extends Db_ActiveRecord
	{
		...
		public function after_create()
		{
			...
			if (!$this->blog_owner_comment && $configObj->comment_notifications_rule != Blog_Configuration::notify_nobody)
			{
				...
				{
					$user = Users_User::create()->find($post->created_user_id);
					if ($user && $user->get_permission('blog', 'notify_blog_comments', true))
						Core_Email::sendOne('blog', 'new_comment', $viewData, 'New comment in blog', $post->created_user_id);
				} elseif ($configObj->comment_notifications_rule == Blog_Configuration::notify_all)
				{
					$users = Users_User::list_users_having_permission('blog', 'notify_blog_comments', true);
		...
		public function before_save($deferred_session_key = null)
		{
			if (strlen($this->content))
				$this->content_html = Phpr_Html::paragraphize(h($this->content));
		}
