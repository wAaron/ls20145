<?
	...
	class Session_Handle extends Backend_Controller
	{
		...
		public function create($path)
		{
			...
			$agent = strtolower(Phpr::$request->getUserAgent());
			if($is_ie = !preg_match('/opera|webtv/i', $agent) && preg_match('/msie\s(\d)/i', $agent))
			{
				//allow when using IE > 9
				$matches = array();
				if (preg_match('/msie\s([0-9\.]+)/i', $agent, $matches))
				{
					if ((float)$matches[1] >= 9)
						$is_ie = false;
				}
			}
		...
		protected function restore_finish_onRestore($hash)
		{
			try
			{
				...
				$user->init_columns_info();
				$user->define_form_fields();
