...
var Overlay = new Class({
	...
	build: function() 
	{
		if (!this.overlay) 
		{
			...
			}).inject(document.body, 'top').addClass('ui-overlay');
...
PopupForm = new Class({
	...
	show: function()
	{
		...
		if (this.options.closeByEsc)
	    	$(document).addEvent('keydown', this.cancelByKey);
		...
		loadIndicator: {show: false}, onSuccess: this.formLoaded.bind(this)}).post({phpr_popup_form_request: 1});
	},
	...
	cancel: function() 
	{
		...
		{
			this.destroy();
		    $(document).removeEvent('keydown', this.cancelByKey);
			return false;
		}
	},
