...
(function( $, undefined ) {
	jQuery.ui.grid = {editors: {}};
	
	jQuery.ui.grid.editors.editorBase = jQuery.Class.create({
		initialize: function() {
		},
		...
		initContent: function(cell, cellContent, row, cellIndex, columnInfo, rowDataIndex) {
			if (cell.find('.cell-content-container').length == 0)
				cell.append(jQuery('<div/>').addClass('cell-content-container').attr('tabindex', 0));
		...
		createCellValueElement: function(cell, cellIndex, rowDataIndex) {
			...
			contentContainer.append(jQuery('<input type="hidden" class="cell-value"></input>').attr(
				'name', 
				grid.options.name+'['+rowDataIndex+']['+grid.options.columns[cellIndex].field+']'
			));

			contentContainer.append(jQuery('<input type="hidden" class="internal-value"></input>').attr(
				'name', 
		...
		renderContent: function(cell, cellContent, row, cellIndex) {
			if (jQuery.type(cellContent) !== 'object') {
		...
		setCellDisplayText: function(cell, text) {
			...
			if (span.length == 0)
			{
				...
				span = jQuery('<span class="cell-content"></span>').text(text);
		 ...
		 * @param object contentContainer Specifies the content containier jQuery object.
	...
	jQuery.ui.grid.editors.popupEditor = jQuery.Class.create(jQuery.ui.grid.editors.editorBase, {
		initContent: function(cell, cellContent, row, cellIndex, columnInfo, rowDataIndex) {
			var self = this;
		...
		getZIndex: function(element) {
			var zElement = element.parents().filter(function(){ 
				var css = jQuery(this).css('z-index');
		...
		buildPopup: function(cell, columnInfo, rowDataIndex, editor) {
			...
			var overlay = jQuery('<div class="ui-grid-overlay ui-overlay"/>');
			if (zIndex)
				overlay.css('z-index', zIndex+1);
			
			jQuery(grid.element).append(overlay);
			jQuery.ui.grid.popupOverlay = overlay;
			
			/*
			 * Build the popup window
			 */
			
			var tmp = jQuery('<div/>').css('display', 'none');
			jQuery(grid.element).append(tmp);
			grid._hideRowMenu();
			...
			new Request.Phpr({
				...
				handler: grid.actionName+'onFormWidgetEvent', 
				extraFields: {
				...
				onFailure: function(xhr) {
					...
					tmp.remove();
					overlay.remove();
					jQuery.ui.grid.popupOverlay = undefined;
				},
			}).post(cell.closest('form')[0]);
		},
		
		displayPopup: function(dataContainer, zIndex, cell, grid) {
			var 
				container = jQuery('<div class="ui-popup-container"/>').css({
					'z-index': zIndex+2,
					'visibility': 'hidden'
				}),
				handle = jQuery('<div class="ui-popup-handle"/>'),
				content = jQuery('<div class="ui-popup-content-container"/>'),
				containerOffset = jQuery(grid.element).offset(),
			...
			jQuery.ui.grid.popupContainer = container;
			jQuery.ui.grid.popupCell = cell;
			...
			jQuery(grid.element).append(container);
			...
			jQuery(document).bind('keydown.gridpopup', function(ev){
				if (ev.keyCode == 27)
	...
	jQuery.ui.grid.editors.textEditor = jQuery.Class.create(jQuery.ui.grid.editors.popupEditor, {
		initialize: function() {
		...
		displayEditor: function(cell, navigationDirection, selectAll, columnInfo) {
			...
			var self = this,
 				editor = jQuery('<input type="text" class="cell-editor"/>')
				.attr('value', this.getCellValue(cell))
				.css('height', cell.height()+'px'),
				editorContainer = jQuery('<div class="cell-editor-container"/>'),
			...
			if (columnInfo.editor_class !== undefined) {
				var 
					button = jQuery('<a href="#" class="ui-grid-cell-button"></a>'),
					knob = jQuery('<span></span>'),
					heightTweak = -1,
					row = cell.parent();
		...
		autocomplete: function (request, response, columnInfo, cell, row, url, form, grid) {
			...
			var lastXhr = jQuery.ajax({
				...
				headers: {
					'PHPR-REMOTE-EVENT': 1,
					'PHPR-POSTBACK': 1,
					'PHPR-EVENT-HANDLER': 'ev{'+grid.actionName+'onFormWidgetEvent}',
	 ...
	 */	
	jQuery.ui.grid.editors.dropdownEditor = jQuery.Class.create(jQuery.ui.grid.editors.editorBase, {
		initialize: function() {
			this.base('initialize');
		},
		...
		displayEditor: function(cell, navigationDirection, columnInfo) {
			...
			var select = jQuery('<select>'),
				selectContainer = jQuery('<div class="ui-grid-select-container">');
			...
			jQuery.each(columnInfo.option_keys, function(index, value){
				select.append(jQuery('<option></option>').attr('value', value).text(columnInfo.option_values[index]));
				if (currentValue == value)
					valueFound = true;
			});
			
			if (!valueFound)
				select.append(jQuery('<option></option>').attr('value', currentValue).text(currentValue));
			...
			window.setTimeout(function(){
				jQuery(document).bind('mousedown.gridDropdown', function(e) { 
//					self.hideEditor(cell, select);
				});
			}, 300);
		...
		hideEditor: function(cell, editor) {
			...
			jQuery(document).unbind('.gridDropdown');
	...
	jQuery.ui.grid.editors.checkboxEditor = jQuery.Class.create(jQuery.ui.grid.editors.editorBase, {
		initContent: function(cell, cellContent, row, cellIndex, columnInfo, rowDataIndex) {
			var self = this;
			this.base('initContent', cell, cellContent, row, cellIndex, columnInfo, rowDataIndex);
			...
			this.hideCellDisplayText(cell);
			var editor = jQuery('<div class="checkbox"/>')
				.attr('tabindex', 0)
				.bind('click', function(){
	...
	jQuery.ui.grid.hidePopup = function() {
		if (!jQuery.ui.grid.popupContainer !== undefined && jQuery.ui.grid.popupOverlay !== undefined)
		{
			hide_tooltips();
			jQuery.ui.grid.popupContainer.remove();
			jQuery.ui.grid.popupOverlay.remove();
			jQuery.ui.grid.popupCell.removeClass('popup-focused');
			jQuery.ui.grid.popupCell.data('ui.grid')._enableRowAdding();
			if (jQuery.ui.grid.popupCell.hasClass('ui-grid-cell-focusable'))
				jQuery.ui.grid.popupCell.find('.cell-content-container').focus();
			
			jQuery(document).unbind('keydown.gridpopup');
		}
	}
