	...
	jQuery.widget( "ui.grid", {
		...
		alignColumns: function(row) {
			...
			this.headTable.find('th div.ui-grid-head-content').each(function(index, div){
				widths.push(jQuery(div).outerWidth());
			});
			...
			if (row === undefined) {
				this.bodyTable.find('tr:first-child').each(function(rowIndex, row) {
					$(row).find('td').each(function(index, td) {
						 if (index < (widths.length-1)) {
							...
							if (Browser.Engine.webkit && !self.isChrome && index > 0)
								tweak = 1;
				...
				row.find('td').each(function(index, td) {
					if (index < (widths.length-1)) {
						...
						jQuery(td).css('width', (widths[index] + tweak) + 'px');
					}
				})
			}
		},
		...
		addRow: function(data, position) {
			...
			this._setCurrentRow(jQuery(row));
			
			if (this.options.useDataSource) {
				var recordCount = parseInt(this.element.find('.pagination .row_count').text());
				if (recordCount != NaN)
					this.element.find('.pagination .row_count').text(recordCount+1);
			}
		...
		appendRow: function(currentCell, ev) {
			...
			var cell = currentCell !== undefined ? currentCell : ((ev !== undefined && ev.target !== undefined) ? jQuery(ev.target).closest('table.ui-grid-body tr td') : null);
		...
		_create: function() {
			jQuery(this.element).data('ui.grid', this);
			this.isChrome = jQuery('body').hasClass('chrome');
			...
			jQuery(window).bind('phpr_widget_response_data', function(event, data){
		...
		_updateScroller: function(direction){
			if (this.options.scrollable)
				this.scroller.update(direction);
		},
		
		_buildHead: function() {
 			if (!this.options.useDataSource) {
				this.headTable = jQuery('<table class="ui-grid-head"></table>');
				var 
					row = jQuery('<tr></tr>')

				this.headTable.append(jQuery('<thead></thead>').append(row));
				
				jQuery.each(this.options.columns, function(index, columnData){
					var 
						cell = jQuery('<th></th>'),
						content = jQuery('<div class="ui-grid-head-content"></div>');
		...
		_buildBody: function() {
			...
			this.bodyTable = this.existingTable.length ? this.existingTable.find('tbody') : jQuery('<table class="ui-grid-body"></table>');

			if (!this.existingTable.length) {
				jQuery.each(this.options.data, function(rowIndex, rowData) {
				...
				if (!this.options.useDataSource || this.built === undefined)
					this.bodyTable.bind('mouseover', function(ev){
						if (ev.target !== undefined)
							self._initRow(jQuery(ev.target).closest('tr'));
					});
			}

			if (this.options.scrollable) {
				this.scrollablePanel = this.element.find('div.backend_scroller');
				this.scroller = new BackendVScroller(this.scrollablePanel[0], {
			...
			if (this.options.sortable) {
				...
				jQuery(document).bind('mousemove', function(event){self._mouseMove(event);});
				jQuery(document).bind('mouseup', function(event){self._mouseUp(event);});
			}
		...
		_buildRow: function(rowData, position) {
			...
			jQuery.each(rowData, function(cellIndex, cellData){
				var cell = jQuery('<td></td>').data('ui.grid', self);
		...
		_createRowMenu: function(row, cell, grid) {
			var 
				...
				menu = jQuery('<span class="ui-grid-menu"></span>'),
				deleteLink = jQuery('<a class="menu-delete">Delete</a>').attr('tabindex', 0).bind('click', {grid: grid}, function(ev){
		...
		_assignDocumentHandlers: function() {
			...
			jQuery(document).bind('keydown.grid', function(ev){
		...
		_assignBodyEventHandlers: function() {
			...
			this.bodyTable.bind('keydown', function(ev){
				if (ev.keyCode == 9) {
					...
					if (ev.target !== undefined && jQuery(ev.target).hasClass('cell-content-container')) {
						if (!ev.shiftKey)
							self._navigateRight(jQuery(ev.target).parent(), false);
						else
							self._navigateLeft(jQuery(ev.target).parent(), false);
				...
				if ((ev.metaKey || ev.ctrlKey)) {
					if (ev.target !== undefined) {
						...
						var cell = jQuery(ev.target).closest('table.ui-grid-body tr td');
						...
						if (ev.keyCode == 68) {
							...
							var 
								row = jQuery(ev.target).closest('table.ui-grid-body tr'),
		...
		_bindSearch: function() {
			this.searchField = jQuery('#' + this.element.attr('id') + '_search_field')
		...
		_rowMouseEnter: function(event){
			...
			jQuery(event.currentTarget).find('span.ui-grid-menu').removeClass('menu-visible');
		...
		_disableRowMenu: function(cell) {
			this._clearMenuTimer();
			this._hideRowMenu();
			jQuery(cell).closest('tr').addClass('ui-no-menu');
		},
		
		_enableRowMenu: function(cell) {
			jQuery(cell).closest('tr').removeClass('ui-no-menu');
		},
		...
		_mouseMove: function(event) {
			...
			if (!this.dragStarted) {
				...
				jQuery(this.dragRow).addClass('ui-selected');
			}
			...
			if (this.options.scrollable) {
				if (this.viewportOffset.top > event.pageY) {
					...
					this.scroller.update_position(this.scrollablePanel.scrollTop()-50);
		...
		_mouseUp: function(event) {
			...
			this.dragging = false;
			jQuery(this.dragRow).removeClass('ui-selected');
		...
		_getColumnEditor: function(cellIndex) {
			...
			return this.editors[editorName] = new jQuery.ui.grid.editors[editorName]();
		},
		...
		_navigateRight: function(fromCell, selectAll) {
			...
			if (next.length) {
				jQuery(next[0]).trigger('navigateTo', ['right', selectAll]);
				return true;
			}
			else {
				var 
		...
		_navigateLeft: function(fromCell, selectAll) {
			...
			if (prev.length) {
				jQuery(prev[0]).trigger('navigateTo', ['left', selectAll]);
				return true;
			}
			else {
		...
		_markErrorField: function(field) {
			...
			if (this.options.scrollable)
			{
				...
				this.scroller.update_position(offset);
			}
		...
		_gotoPage: function(pageIndex, options) {
			...
			var 
				searchTerm = this.searchField !== undefined ? this.searchField.val() : null,
				data = $merge({
			...
			new Request.Phpr({
				...
				handler: this.actionName+'onFormWidgetEvent', 
				extraFields: data, 
				update: 'multi', 
			...
			}).post(this.element.closest('form')[0]);
		...
		_showNoSearchResults: function() {
			...
			if (this.noSearchResultsMessage === undefined) {
				this.noSearchResultsMessage = jQuery('<p class="ui-no-records noData"></p>').text('No records found');
		...
		_initHeaderCheckboxes: function() {
			...
			this.headTable.find('th.checkbox-cell input').each(function(index, input) {
				input.addEvent('click', function() {
					var headCell = jQuery(input).closest('th');
					...
					self.bodyTable.find('tr').find('td:eq('+cellIndex+')').each(function(rowIndex, cell){
						if (input.checked)
							jQuery(cell).find('div.checkbox').addClass('checked');
						else
							jQuery(cell).find('div.checkbox').removeClass('checked');
							
						jQuery(cell).find('.cell-value').val(input.checked ? 1 : 0);
