<?
	...
	class Db_GridWidget extends Db_FormWidget
	{
		...
		/**
		 * @var boolean Determines whether the toolbar has the Add Row button.
		 */
		public $toolbar_add_button = true;

		/**
		 * @var boolean Determines whether the toolbar has the Delete Row button.
		 */
		...
		{
			parent::__construct($controller, $model, $column_name, $configuration);
			
			if (!$this->max_row_number)
				$this->max_row_number = Phpr::$config->get('GRID_MAX_ROW_NUMBER', 300);
		}
		...
		protected function on_csv_file_uploaded($field, $model)
		{
			try
			{
				...
				try
				{
					...
					$this->controller->preparePartialRender($container_id);
