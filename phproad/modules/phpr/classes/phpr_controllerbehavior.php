<?php
	...
	class Phpr_ControllerBehavior extends Phpr_Extension
	{
		...
		/**
		 * This method allows to add global event handlers to the behavior.
		 * @param string $eventName Specifies an event name. The behavior class must contain a method with the same name.
		 */
		protected function addGlobalEventHandler($eventName)
		{
			$this->_controller->addGlobalEventHandler($eventName);
			$this->_controller->addDynamicMethod($this, $eventName, $eventName);
		}
		...
		private function renderPartialFile($controllerViewPath, $viewName, $params = array(), $overrideController = false, $throwNotFound = true)
		{
			...
			{
				if (strpos($viewName, '/') !== false && file_exists($viewName.'.htm'))
				{
					$this->_controller->renderPartial($viewName.'.htm', $params, true, true);
					return;
				}
